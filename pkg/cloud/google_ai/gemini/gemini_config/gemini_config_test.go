...
func TestNew(t *testing.T) {
	...
	if defaults.ApiKey != "" {
		t.Errorf("default ApiKey = %q, want empty", defaults.ApiKey)
	}
	if len(defaults.FetchOptions) != 0 {
		t.Errorf("default FetchOptions len = %d, want 0", len(defaults.FetchOptions))
	}

	baseUrl := &url.URL{Scheme: "https", Host: "gemini.example.com"}
	config := New(
		WithBaseUrl(baseUrl),
		WithApiKey("secret"),
	...
	if config.ApiKey != "secret" {
		t.Errorf("ApiKey = %q, want %q", config.ApiKey, "secret")
	}
...
func TestWithFetchOptionsAppends(t *testing.T) {
	t.Parallel()

	config := New(
		WithFetchOptions(fetch_config.WithMethod(http.MethodPut)),
		WithFetchOptions(fetch_config.WithMethod(http.MethodDelete)),
	)
	if len(config.FetchOptions) != 2 {
		t.Fatalf("FetchOptions len = %d, want 2", len(config.FetchOptions))
	}
	// Options are applied in order, so the last one wins.
	if applied := fetch_config.New(config.FetchOptions...); applied.Method != http.MethodDelete {
		t.Errorf("applied fetch Method = %q, want %q", applied.Method, http.MethodDelete)
	}
}
