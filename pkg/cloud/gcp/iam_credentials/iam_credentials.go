...
func (c *Client) SignBlob(ctx context.Context, serviceAccountEmail string, payload []byte, options ...fetch_config.Option) (*sign_blob_response.Response, error) {
	...
	_, response, err := altshiftHttpUtils.FetchJsonWithBody[*sign_blob_response.Response](ctx, urlString, body, options...)
	if err != nil {
		return nil, altshiftErrors.New(fmt.Errorf("fetch json with body: %w", err), urlString)
	}
	if response == nil {
		return nil, altshiftErrors.NewWithTrace(nil_error.New("sign blob response"))
	}
...
func (c *Client) SignJwt(ctx context.Context, serviceAccountEmail string, payload string, options ...fetch_config.Option) (*sign_jwt_response.Response, error) {
	...
	if payload == "" {
		return nil, altshiftErrors.NewWithTrace(empty_error.New("payload"))
	}
	...
	options = append(append(c.config.FetchOptions, options...), fetch_config.WithMethod(http.MethodPost))
	_, response, err := altshiftHttpUtils.FetchJsonWithBody[*sign_jwt_response.Response](ctx, urlString, body, options...)
	if err != nil {
		return nil, altshiftErrors.New(fmt.Errorf("fetch json with body: %w", err), urlString)
	}
	if response == nil {
		return nil, altshiftErrors.NewWithTrace(nil_error.New("sign jwt response"))
	}
